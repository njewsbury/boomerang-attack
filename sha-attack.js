...
ca.crypto.research.ShaAttack = (function () {
     ...
     * 
     * @param precomputedMessage {string} Hexadecimal string of the message
     * @param logger {log function} Log function to display information to the user
     * @oaram numRounds {int (optional)} Number of rounds to run SHA-0 hash operation
     *
     * @return {boolean} If a collision was found.
     */
     ...
     *
     * @param exponent {int} The number of collisions to calculate as a function of 2^exponent
     * @param blockCount {int (optional)) How many blocks the messages should be, 
     *  default to one.
     * @param logger {function} Function to use to log results
     * @param numRounds {int (optional)} The number of rounds to use in SHA-0
     *
     * @return {boolean} If a collision was found.
     */
    function beginAttack(exponent, blockCount, logger, numRounds) {
      ...
      // Timing      
      totalDiff = (Date.now() - startTotalHash)/1000; //seconds.
      logger(
        'Calculated ' 
        + hashes 
        + ' hashes in '
        + Math.round(totalDiff/60/60) + ' hours '
        + Math.round(totalDiff/60) + ' mins ' 
        + (totalDiff%60) + ' sec'
        + ' RAW: ' + totalDiff
      );
      //
